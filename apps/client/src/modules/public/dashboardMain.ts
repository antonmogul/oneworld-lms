...
export const userDashboardMain = () => {
  ...
  const restartLessonReq = publicQL.mutation(RestartLessonDocument);
      ...
      if (lessonProgress && lessonProgress.completed) {
        ...
        startCourseLink.on("click", () => {
          startCourseLink
            ...
            .setText("Please wait...");
            restartLessonReq.onData((res) => {
            if (res) {
              navigate(`${PUBLIC_PATHS.viewCourse}?id=${rowData.id}`);
            }
          });
          restartLessonReq.fetch({
            lessonId: rowData.id
          })
        });
    ...
    getAllLessonProgressReq.onData(async (progressData) => {
      getAllCertificationsReq.onData(async (certificationsData) => {
        if (progressData.getAllLessonProgress) {
          ...
          const allEnabledCertifications = certificationsData.publicGetAllCertifications.filter(c => c.lesson.some(l => l.enabled));
          let currentCertification = certificationsData.publicGetAllCertifications.filter(c => c.lesson.filter(l => (l.lessonProgress[0] && (!l.lessonProgress[0].completed) || !l.lessonProgress[0])).length)[0];
          currentCertification = currentCertification ? currentCertification : allEnabledCertifications[allEnabledCertifications.length - 1];
          const completedLessons = currentCertification.lesson.filter(l => l.enabled && l.lessonProgress && l.lessonProgress.length && l.lessonProgress[0] && l.lessonProgress[0].completed);
          ...
          for(let l of allLessons) {
            ...
            if (lesson) {
              ...
              if (totalSlides && progress && totalSlides === progress.progress + 1 && progress.completed) {
          ...
          const currentCourseId = await setCurrentCourse(coursesData.getAllLessons, null, certificationsData.publicGetAllCertifications);
          if (currentCourseId) {
...
const setCurrentCourse = (coursesData, progressData, allCertificationsData) => {
  return new Promise((resolve, reject) => {
    ...
    if (progressData && progressData.length) {
      ...
      if (inCompleteProgressData && inCompleteProgressData.length) {
        ...
        currentCertification = allCertificationsData.filter(c => c.lesson.filter(l => (l.lessonProgress[0] && (!l.lessonProgress[0].completed) || !l.lessonProgress[0])).length)[0];
        ...
        try {
          ...
          dashboardHeadingM.getElement().innerHTML = `${removeH5Tags(currentCertification.title)}`;
        } catch(err) {
          console.log(err);
        }
        ...
        if (coursesNotStarted && coursesNotStarted.length) {
          ...
          currentCertification = allCertificationsData.find((c) => !!c.lesson.find((l) => l.id === currentCourse.id));
          totalLessonsOfCurrentCertification = currentCertification.lesson.filter((lesson) => lesson.enabled).length;
          completedLessonsOfCurrentCertification = currentCertification.lesson.filter((lesson) => lesson.enabled && lesson.lessonProgress[0] && lesson.lessonProgress[0].completed).length;
          currentCourseImage.setAttribute(
            "src",
          ...
          let completedCourses = coursesData.filter((c) => {
            if (completedCourseIds.includes(c.id)) {
              return true;
            } else {
              return false;
          ...
          currentCourse = completedCourses[completedCourses.length - 1];
          currentCourse.isStarted = false;
          currentCertification = allCertificationsData.find((c) => !!c.lesson.find((l) => l.id === currentCourse.id));
          totalLessonsOfCurrentCertification = currentCertification.lesson.filter((lesson) => lesson.enabled).length;
          completedLessonsOfCurrentCertification = currentCertification.lesson.filter((lesson) => lesson.enabled && lesson.lessonProgress[0] && lesson.lessonProgress[0].completed).length;
          ...
          try {
            ...
            courseHeading.getElement().innerHTML = removeH5Tags(currentCertification.title);
            const motivationLine = new WFComponent(`[xa-var="motivation-line"]`);
            motivationLine.getElement().innerHTML= removeH5Tags(`- Restart the course!`);
            const motivationLineM = new WFComponent(`[xa-var="motivation-lineM"]`);
            motivationLineM.getElement().innerHTML= removeH5Tags(`- Restart the course!`);
          } catch(err) {
            console.log(err);
          }
          currentCourseDiv.updateTextViaAttrVar({
            // courseName: currentCourse.title,
            btnText: "Restart Here",
          });
          continueCourseLink.on("click", () => {
            restartLessonReq.onData((res) => {
              if (res) {
                navigate(`${PUBLIC_PATHS.viewCourse}?id=${currentCourse.id}`);
              }
            });
            restartLessonReq.fetch({
              lessonId: currentCourse.id
            })
          });
