...
const adminHeader = () => {
  const adminName = new WFComponent(
    `[xa-type="admin-name"]`
  );
  const logout = new WFComponent<HTMLAnchorElement>(
    `[xa-type="logout"]`
  );
  const userDetails = adminAuth.getUser();
  // adminName.setText(`${userDetails.name.split(" ")[0]}`);
  logout.on("click", () => {
    logoutAdminAuth();
  });
